...
class LiaisonAssumptionController extends Controller
{
    ...
    public function showForm(Request $request, SystemController $sys)
    {
        ...
        if ($status == 1) {
            ...
            return view('liaison.assumptionForm')->with('data', $query)
    ...
    public function processForm(Request $request, SystemController $sys)
    {
        ...
        if (empty($checkQuery)) {
            ...
            $data->company_exact_location = ucwords($clocation);
            ...
            $data->company_town = ucwords($ctown);
            $data->company_supervisor_phone = ucwords($csphone);
            ...
            $sql = Models\AssumptionDutyModel::where("indexno", $indexno)
                ->update(array(
                    "company_name" => ucwords($cname),
                    "company_phone" => $cphone,
                    "company_exact_location" => ucwords($clocation),
                    ...
                    "company_supervisor_phone" => ucwords($csphone),
                    "company_town" => ucwords($ctown),
                    "date_duty" => $cdate,
